...
files (GitHub Pages style) and must keep working with **no build step, no framework, no bundler** —
...
- **Open state** is the `.modal.is-open` class — never toggle `style.display` directly.
- **Lazy galleries:** images inside modal `.gallery-grid`s use `data-src` (plus a tiny inline-SVG placeholder
  ...
  does NOT work inside `display:none` containers; the `data-src` swap is deliberate.
...
- The contact form posts through EmailJS; feedback goes to `#form-status` (`aria-live`), never `alert()`.
  The EmailJS public key in `main.js` is public by design (browser SDK) — it is not a leaked secret.
...
- Element `id`s are unique (this was once broken — validators and the modal system rely on uniqueness).
- Heading hierarchy: one `<h1>` (hero name); each section title is `<h2>`; card/entry titles are `<h3>`/`<h4>`.
- Dates use `<p class="date">` with en dashes ("June 2023 – August 2023").
...
- Suncorp and Duo are private client sites — their cards deliberately open a "private" notice instead of linking out.
...
- Git history still contains ~40 MB of pre-compression images. Cleaning it requires history rewriting
  (`git filter-repo`) — destructive, only with the owner's explicit go-ahead.
